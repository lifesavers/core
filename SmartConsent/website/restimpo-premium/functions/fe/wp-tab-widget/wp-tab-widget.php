<?php
/*
Plugin Name: RestImpo Tab Widget
...
class restimpo_widget extends WP_Widget {
	function __construct() {
        
		// ajax functions
		add_action('wp_ajax_restimpo_widget_content', array(&$this, 'ajax_restimpo_widget_content'));
		add_action('wp_ajax_nopriv_restimpo_widget_content', array(&$this, 'ajax_restimpo_widget_content'));
		...
		$widget_ops = array('classname' => 'widget_wpt', 'description' => __('Display popular posts, recent posts, comments and tags in tabbed format.', 'restimpo'));
		$control_ops = array('width' => 300, 'height' => 350);
		$this->WP_Widget('restimpo_widget', __('RestImpo Tab Widget', 'restimpo'), $widget_ops, $control_ops);
    }	

    function wpt_admin_scripts($hook) {
        wp_enqueue_script( 'restimpo_widget_admin', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wpt-admin.js', array( 'jquery' ), '1.0', true );
    }
    function wpt_register_scripts() { 
		// JS    
		wp_enqueue_script('restimpo_widget', get_template_directory_uri() . '/functions/fe/wp-tab-widget/js/wp-tab-widget.js', array('jquery'), '1.0', true);     
		wp_localize_script( 'restimpo_widget', 'wpt',         
	...
	function form( $instance ) {
        ...
        <h4><?php _e('Select Tabs', 'restimpo'); ?></h4>
		...
		<div class="wpt_select_tabs">
			...
			<label class="alignleft" style="display: block; width: 50%;" for="<?php echo $this->get_field_id("tabs"); ?>_tags">
				...
				<?php _e( 'Tags Tab', 'restimpo'); ?>
			</label>
		</div>
        <div class="clear"></div>
        
        <h4 class="wpt_tab_order_header"><a href="#"><?php _e('Tab Order', 'restimpo'); ?></a></h4>
        ...
        <div class="wpt_tab_order" style="display: none;">
            ...
            <label class="alignleft" for="<?php echo $this->get_field_id('tab_order'); ?>_tags" style="width: 50%;">
			    ...
			    <?php _e('Tags', 'restimpo'); ?>
            </label>
        </div>
		<div class="clear"></div>
        
        <h4 class="wpt_advanced_options_header"><a href="#"><?php _e('Advanced Options', 'restimpo'); ?></a></h4>
        ...
        <p>
			<label for="<?php echo $this->get_field_id("allow_pagination"); ?>">				
				...
				<?php _e( 'Allow pagination', 'restimpo'); ?>
			</label>
		</p>
        ...
        <p>
			<label for="<?php echo $this->get_field_id('post_num'); ?>"><?php _e('Number of posts to show:', 'restimpo'); ?>
				<br />
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_thumb"); ?>">
				...
				<?php _e( 'Show post thumbnails', 'restimpo'); ?>
			</label>
		</p>   	
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_date"); ?>">	
				...
				<?php _e( 'Show post date', 'restimpo'); ?>	
			</label>	
		</p>
		...
		<p>		
			<label for="<?php echo $this->get_field_id("show_comment_num"); ?>">		
				...
				<?php _e( 'Show number of comments', 'restimpo'); ?>		
			</label>	
		</p>    
        ...
        <p>
			<label for="<?php echo $this->get_field_id('comment_num'); ?>">
				<?php _e('Number of comments on Comments Tab:', 'restimpo'); ?>
				<br />
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_avatar"); ?>">			
				...
				<?php _e( 'Show avatars on Comments Tab', 'restimpo'); ?>	
			</label>	
		</p>
	...
	function widget( $args, $instance ) {	
		extract($args);     
		extract($instance);    
		wp_enqueue_script('restimpo_widget');  
        ...
        $available_tabs = array('popular' => __('Popular', 'restimpo'), 
            'recent' => __('Recent', 'restimpo'), 
            'comments' => __('Comments', 'restimpo'), 
            'tags' => __('Tags', 'restimpo'));
            
        array_multisort($tab_order, $available_tabs);
        
		?>	
		<?php echo $before_widget; ?>	
		<div class="restimpo_widget_content" id="<?php echo $widget_id; ?>_content">		
	...
	function ajax_restimpo_widget_content() {     
		$tab = $_POST['tab'];       
		$args = $_POST['args'];  
		...
		switch ($tab) {        
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>	
								<div class="wpt-postmeta">						
									...
									<?php if ( $show_comment_num == 1 ) : ?>			
										<?php echo comments_number(__('No Comment','restimpo'), __('One Comment','restimpo'), '<span class="comments-number">%</span> '.__('Comments','restimpo'));?>				
									<?php endif; ?>						
								</div> <!--end .entry-meta--> 				
							<?php endif; ?>
						...
						<li>
							...
							<?php if ( $show_date == 1 || $show_comment_num == 1) : ?>			
								<div class="wpt-postmeta">										
									...
									<?php endif; ?>								
									<?php if ( $show_comment_num == 1 ) : ?>	
										<?php echo comments_number(__('No Comment','restimpo'), __('One Comment','restimpo'), '<span class="comm">%</span> '.__('Comments','restimpo'));?>									
									<?php endif; ?>		
								</div> <!--end .entry-meta--> 		
							<?php endif; ?>
			...
			case "comments":         
				...
				<ul>            
					...
					<?php endforeach; else : ?>           
						<li>                   
							<div class="no-comments"><?php _e('No comments yet.', 'restimpo'); ?></div>        
						</li>                             
    ...
    function tab_pagination($page, $last_page) {  
		...
		<div class="wpt-pagination">     
			<?php if ($page > 1) : ?>               
				<a href="#" class="previous"><span><?php _e('&larr; Previous', 'restimpo'); ?></span></a>      
			<?php endif; ?>        
			<?php if ($page != $last_page) : ?>     
				<a href="#" class="next"><span><?php _e('Next &rarr;', 'restimpo'); ?></span></a>      
			<?php endif; ?>          
		</div>                   
		<div class="clear"></div>
...
add_action( 'widgets_init', create_function( '', 'register_widget( "restimpo_widget" );' ) );
